export const projectData = {
  ...
  team: {
    수상한자판기: {
      ...
      details:
        "수상한 자판기는 첫 번째 팀 프로젝트로, 팀원들과 함께 비현실적인 물건 판매라는 테마를 기획했다. <br/>각자의 아이디어를 통합해 5개의 테마를 가진 물건을 판매하는 방식으로 구성했고 각 테마마다 다른 배경을 설정해 시각적인 차별화를 주어 프로젝트를 더 흥미롭게 만들었다.<br/><br/>프로젝트에서는  <strong>Sqlite3을 사용해 데이터를 관리</strong>했으며,  <strong>Vanilla Node.js를 이용해 필요한 기능만 최소화해 서버를 구현</strong>했다. <br/>이 과정에서 비동기 처리 개념을 배우고 이를 활용해 데이터베이스 연동을 효과적으로 처리했다.<br/><br/>내가 맡은 작업은 <strong>DB 생성 및 관리와 서버 구현</strong>이었다. <br/>또한 <strong>cookie 개념을 학습하여 로그인 기능을 구현</strong>했고 간단한 프론트 로직을 사용해 로그인 페이지를 제작했다.<br/><br/>프론트엔드 디자인에도 적극적으로 참여하며 프론트엔드 담당 팀원과 소통해 테마 배경과 로직 구성에 대해 논의하고 조율했다.",
      data: [
        "/img/자판기_main.png",
        "/img/자판기_login.png",
        "/img/자판기_테마1.png",
    ...
    DelaN: {
      ...
      content:
        "관리자는 개발자라는 인적 자원을 관리하고, 개발자는 자신을 관리할 수 있는 사이트를 목표했다.<br/> 그를 위해 관리자 페이지와 사용자 페이지로 분리해 로그인 한 사용자의 권한 레벨에 따라 해당 페이지에서 사용할 수 있도록 했다. <br/>관리자 페이지는 프로젝트 생성, 팀 생성, 근태 확인, 회원가입 승인, 권한 부여, DB GUI와 같은 관리 기능을 포함하고 있다. <br/>사용자 페이지는 개인 프로필 관리, 프로젝트 조회 및 TodoList 관리, 출근 및 퇴근 표시와 같은 기능을 포함하고 있다.",
      details: ["ERD 구조 반영 및 문서 제작 상세 설명"],
      data: [],
      imgnavigate: {},
      review: "",
      githubLink: [
