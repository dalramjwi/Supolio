export const projectData = {
  ...
  team: {
    ...
    호랑이: {
      ...
      description:
        "머신 러닝을 활용해 예측 모델을 제작하고자 했다.<br/> 이를 통해 사람 이미지 데이터를 분석하여 가상 아바타의 스테이터스를 추출하고 시뮬레이션 모델에서 해당 아바타가 수행한 활동을 사용자에게 제공하는 웹페이지를 제작 중이다.",
      stack: [
        "Python",
        "FastAPI",
        "TensorFlow",
      ...
      content:
        "이 서비스에서는 두 가지 모델이 사용된다. <br/>첫 번째는 이미지 분석 모델로, 사용자가 입력한 사람 이미지를 바탕으로 힘, 민첩성과 같은 스테이터스를 추출한다. <br/>두 번째는 시뮬레이션 모델로, 앞서 분석된 이미지를 바탕으로 제작된 아바타가 시뮬레이션 환경에서 어떤 결과를 도출하는지를 제공한다. <br/>이 두 모델을 결합하여, 사용자가 제공한 이미지를 기반으로 시뮬레이션 결과를 시각적으로 제공하는 웹 페이지를 제작 중이다.",
      details:
