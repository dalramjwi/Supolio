export const projectData = {
  ...
  team: {
    ...
    이드: {
      ...
      contribute:
        "위키디피아와 같은 온라인 사이트 API를 활용한 동화 데이터 추출",
      content:
        "사용자가 입력한 이야기 데이터를 분석해 이야기 곡선 형태를 추출해내는 데이터 모델을 제작했다.아직 제작 중이다.",
      details:
        "GPT를 접하면서 머신 러닝, 특히 파인 튜닝에 대한 관심이 생겨 이를 직접 시도해보고자 했다. 어떤 데이터를 모델에게 학습시킬지 고민하던 중, 다양한 이야기가 존재하고 저작권이 만료된 동화가 적합하다고 판단하여 동화를 선정하게 되었다. 동화 데이터를 모델에 학습시켜, 커트 보니것의 서사 구조 이론을 바탕으로 이야기의 곡선을 추출해내는 모델을 제작하는 것을 목표했다. 머신러닝과 관련된 지식이 없기 때문에, 기본적인 transformer 모델 관련 지식 학습 및 모델 학습에 대한 개념 학습을 진행하며 제작 중이다. 크롤링을 통해 동화 데이터를 수집하고, 수집한 동화 데이터를 모델을 통해 학습시키며 모델 훈련을 시도하는 중이다.",
      data: [
        "/img/갈라테이아_main.png",
